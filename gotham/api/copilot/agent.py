...
from gotham.api.copilot.tools import TOOL_SCHEMAS, ToolBox
...
def _anthropic_client() -> LLMClient | None:
    key = os.environ.get("GOTHAM_LLM_KEY")
    if not key:
        return None
    try:  # pragma: no cover - exercised only with a real key
        import anthropic
    except ImportError:
        logger.warning("GOTHAM_LLM_KEY is set but the anthropic package is missing")
        return None
...
def answer(
    ...
    model = model or os.environ.get("GOTHAM_LLM_MODEL", DEFAULT_MODEL)
