...
from civictwin.analysis.interventions import generate_catalogue
from civictwin.api.geo import Projector, build_township_payload, projector_for
from civictwin.config import DEFAULT, Config
from civictwin.engine.coordinator import Engine
from civictwin.io import data_version_of, load_township, model_version
...
class AppState:
    ...
    def __init__(
        ...
        self.township_path = Path(
            township_path or os.environ.get("CIVICTWIN_TOWNSHIP", "data/township.json")
        )
        self.results_path = Path(
            results_path or os.environ.get("CIVICTWIN_RESULTS", "results/")
        )
        self.role = role or os.environ.get("CIVICTWIN_ROLE", PLANNER_ROLE)
        self.cfg = cfg
    ...
    def _spill(self, key: str, encoded: str) -> Path:
        ...
        if self._spill_dir is None:
            self._spill_dir = Path(
                tempfile.mkdtemp(prefix="civictwin-scenario-cache-")
            )
    ...
    def copilot_available(self) -> bool:
        return bool(os.environ.get("CIVICTWIN_LLM_KEY"))
